...
def main():
    ...
    if len(datas) > 0:
        if args.pop_ids is None:
            data = datas[0]
            models = [inference.moments_Dplus(g, u=u, data=data, phased=args.phased) 
                      for g in args.graph_files]
        else:
            pop_ids = args.pop_ids
            bins = datas[0]['bins']
            models = [inference.moments_Dplus(g, u=u, bins=bins, sampled_demes=pop_ids, phased=args.phased) 
                      for g in args.graph_files]
        if args.compute_ll:
            lls = [np.round(inference.compute_ll(m, data), 2)
                   for m in models] 
