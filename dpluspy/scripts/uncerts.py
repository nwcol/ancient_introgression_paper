...
from h2py import inference, parsing
    ...
    for in_file in args.data_files:
        ...
        for label in file_regions:
            if label in regions:
                ...
                subset = parsing.subset_H2(file_regions[label], graph=graph)
                regions[label] = subset

    data, raw_reps = parsing.get_bootstrap_reps(regions, mut_weighted=True)
