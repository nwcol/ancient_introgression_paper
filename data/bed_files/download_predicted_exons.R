...
fwrite(pred_genes, "predicted_exons/predicted_genes_hg37.csv")
pred_genes <- fread("predicted_exons/predicted_genes_hg37.csv")
used_genes <- pred_genes 
...
for(c in unique(clipped_genes$chromosome_name)) {
  ...
  fwrite(chr_bed, paste("predicted_exons/exons_chr", c, ".bed", sep=""), sep="\t", col.names=F)
}
