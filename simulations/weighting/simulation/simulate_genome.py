...
use_empirical_map = bool(sys.argv[3])
...
def simulate_chrom(chrom):
    ...
    if use_empirical_map:
        # Simulate mutations
        mts = msprime.sim_mutations(ts, rate=mut_map, record_provenance=False)
    else:
        mts = msprime.sim_mutations(ts, rate=u_bar, record_provenance=False)

    # Compute statistics
    naive_stats = dpluspy.parsing.parse_stats(
        mts,
        bed_file=mask_fname,
    ...
    weighted_stats = dpluspy.parsing.parse_stats(
        mts,
        bed_file=mask_fname,
        rec_map_file=rec_map_fname,
        pos_col="pos",
    ...
    for key in naive_stats:
        weighted_stats[key]["denoms"] = naive_stats[key]["denoms"]
    return naive_stats, weighted_stats
...
for chrom in range(1, 23):
    naive, weighted = simulate_chrom(chrom)
    naive_sums.update(naive)
    weighted_sums.update(weighted)

run_bootstrap(naive_sums, f"{out_fstem}_naive_stats.pkl")
run_bootstrap(weighted_sums, f"{out_fstem}_weighted_stats.pkl")
with open(f"{out_fstem}_naive_sums.pkl", "wb") as fout:
    pickle.dump(naive_sums, fout)
with open(f"{out_fstem}_weighted_sums.pkl", "wb") as fout:
    pickle.dump(weighted_sums, fout)
